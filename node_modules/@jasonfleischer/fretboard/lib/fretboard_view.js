...
class FretboardView {
	...
	draw_background(){
		...
		for(i =0 ; i< note_y_positions.length; i++){
			...
			for(j =0; j <note_x_positions.length; j++){
				...
				ctx.strokeStyle = "#000";
				ctx.lineWidth = 1;
				ctx.arc(note_x_positions[j], note_y_positions[i], this.radius, 0, TWO_PI);
				ctx.fill();
				ctx.stroke();*/
	...
	clearNote(note, canvas = undefined){
		if(canvas === undefined){
			canvas = this.canvas;
		}
	...
	clearHoverNote(note){
		...
		this.clearNote(note, canvas);
	}

	drawNote(note, withLabel = true){

		let canvas = this.canvas;
		var ctx = canvas.getContext("2d");
		var label = withLabel ? note.note_name.type.substring(0, 2) : undefined;
		this.drawNoteWithColor(note, label, note.note_name.color, canvas);
	}

	drawHoverNote(note, withLabel = true, color = "#eee") {
		let canvas = this.canvas_hover;
		let ctx = canvas.getContext("2d");
		let label = withLabel ? note.note_name.type.substring(0, 2) : undefined;
		this.drawNoteWithColor(note, label, color, canvas);
	}

	drawNotePlaceholder(note, label) {
		this.drawNoteWithColor(note, label, "#f00", this.canvas);
	}
	...
	drawNoteWithColor(note, label, color, canvas) {
		...
		for( i=0; i<note_positions.length; i++)  {
			...
			ctx.lineWidth = 1;
			ctx.arc(note_positions[i][0], note_positions[i][1], this.radius, 0, TWO_PI);
	...
	drawChord(chord, withLabels = true){
		let canvas = this.canvas;
		var ctx = canvas.getContext("2d");
		ctx.clearRect(0, 0, this.WIDTH, this.HEIGHT);
		...
		for(j=0; j<note_array.length; j++) {
			...
			if (chord.note_labels[j] == 'R'){
				this.drawNoteWithColor(note, label, note.note_name.color);
			} else {
	...
	drawScale(scale, withLabels = true) {
		let canvas = this.canvas;
		var ctx = canvas.getContext("2d");
		ctx.clearRect(0, 0, this.WIDTH, this.HEIGHT);
		...
		for(j=0; j<note_array.length; j++) {
			var note = note_array[j];
			var label = withLabels ? scale.getLabel(note) : undefined;
			//this.drawNotePlaceholder(note, label);
			if (label == 'R'){
	...
	addClickEventListeners(onClick) {
		...
		view.addEventListener("click", function(event){
			...
			if(foundMidiValue != -1){
				...
				let is_on = self.on_notes.includes(foundMidiValue);
				if (is_on) {
					self.on_notes.splice(self.on_notes.indexOf(foundMidiValue), 1);
				} else {
					self.on_notes.push(foundMidiValue);
				}
		...
		view.addEventListener("mousemove", function(event){
			...
			if(foundMidiValue != -1){
				...
				if(previousMidiValue === undefined) {
		    		...
		    		let withLabel = !self.on_notes.includes(foundMidiValue);
		    		let color = withLabel ? "#eee": "#55000000";
		    		if(!self.on_notes.includes(foundMidiValue))
		    			self.drawHoverNote(musicKit.all_notes[foundMidiValue], withLabel, color);
		    	}
