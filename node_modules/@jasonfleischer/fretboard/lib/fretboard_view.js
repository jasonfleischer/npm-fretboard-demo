		   ...
		   // let clickPoint = new Point(x, y);
		    ...
		    for (const [key, value] of Object.entries(dict)) {
		    	...
		    	for (let i = 0; i < value.length; i++) {
 					...
 					//let point = new Point(note_positions.x, note_positions.y);
 					function distanceBetweenPoints(x1, y1, x2, y2) {
        				return Math.sqrt((Math.pow(x1-x2,2))+(Math.pow(y1-y2,2)));
    				}
 					log.e("r "+ rad);
 					log.e("x "+ x);
 					log.e("x2 "+ note_positions[0]);
 					log.e("y2 "+ note_positions[1]);
 					log.e("d "+ distanceBetweenPoints(note_positions[0], note_positions[1], x, y));
 					if (distanceBetweenPoints(note_positions[0], note_positions[1], x, y) < rad) {
			...
			if(foundMidiValue != -1){
				drawNote(musicKit.all_notes[foundMidiValue]);
			} else {
				log.e("no note found");
			}
		});
