...
class FretboardView {
	...
	clearHoverNote(note){
		...
		for( i=0; i<note_positions.length; i++)  {
			ctx.clearRect(note_positions[i][0]-this.radius, note_positions[i][1]-this.radius, this.radius*2, this.radius*2);
		}
	}
	...
	drawNote(note, withLabel = true){
		...
		this.drawNoteWithColor(note, label, canvas);
	}

	drawHoverNote(note, withLabel = true) {
		let canvas = this.canvas_hover;
		let ctx = canvas.getContext("2d");
		let label = withLabel ? note.note_name.type.substring(0, 2) : undefined;
		this.drawNoteWithColor(note, label, "#f00", canvas);
	}

	drawNotePlaceholder(note, label) {
		this.drawNoteWithColor(note, label, "#555", this.canvas);
	}
	...
	addHoverEventListeners() {
		...
		view.addEventListener("mousemove", function(event){
			...
			if(foundMidiValue != -1){
				...
				self.drawNote(musicKit.all_notes[foundMidiValue]);
			}
		});
